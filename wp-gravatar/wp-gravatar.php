<?php
/*
Plugin Name: wp-gravatar
Author: 夜色静好
Version: 7.0
Description: 一款 Gravatar 头像缓存插件，夜色静好提供汉化。
...
class NFGC_Gravatar_Cache {
    ...
    public $plugin_name = 'wp-gravatar';
    ...
    public function get_cached_avatar( $source, $id_or_email, $size, $default, $alt ) {
        ...
        if (! is_file($g_path) || (time()-filemtime($g_path)) > $time) {
            $curl_url = 'https://secure.gravatar.com/avatar/'.$garvatar_id.'?s='.$size.'&r=G&d='.$g_url_default_sorce;
